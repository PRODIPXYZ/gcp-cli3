...
# ---------- Create 2 VMs in Any Project (Multi-Account) ----------
create_2_vms_in_project() {
    echo -e "\n${CYAN}${BOLD}➕ Create 2 VMs in Any Project (All Accounts)${RESET}\n"

    projects=()
    index=1

    printf "${YELLOW}┌─────┬──────────────────────────────┬───────────────┬───────────────────────┬──────────────────────────────┐${RESET}\n"
    printf "${YELLOW}│%-5s│%-30s│%-15s│%-23s│%-28s│${RESET}\n" "No" "PROJECT" "VM Count" "Billing Status" "ACCOUNT"
    printf "${YELLOW}├─────┼──────────────────────────────┼───────────────┼───────────────────────┼──────────────────────────────┤${RESET}\n"
    ...
    billing_status=$(echo "$selected" | cut -d'|' -f3)
    ...
    echo -e "${GREEN}✔ 2 VMs created successfully in $proj (Account: $acc)!${RESET}"
    read -p "Press Enter to continue..."
}
